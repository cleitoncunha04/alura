...
const editTask = (status, updatedTask) => {
    return {
        ...status,
        tasks: status.tasks.map(task => task === status.selectedTask ? updatedTask : task),
        selectedTask: null,
        isEditing: false
    };
};
const tooggleVisibilityForm = (form) => {
    form.classList.toggle("hidden");
};
const cleanFields = (textArea) => {
    textArea.value = "";
};
const updateUI = () => {
    const taskIconSvg = `
    ...
    `;
    const selectedTaskLabel = document.querySelector('.app__section-active-task-label');
    const deleteCompletedTasksBt = document.querySelector('#btn-remover-concluidas');
    const deleteAllTasksBt = document.querySelector('#btn-remover-todas');
    const ulTaskElement = document.querySelector(".app__section-task-list");
    ...
    const cancelAddTaskBt = document.querySelector('.app__form-footer__button--cancel');
    const cleanTextAreaBt = document.querySelector('.app__form-footer__button--delete');
